...
<div class="container">
    <div class="row justify-content-center">
        <div class="col-md-12">
            <h3 class=" text-center">{{ $sessionName }}</h3>
            <div class="messaging">
                <div class="inbox_msg">
                    <div class="inbox_people">
                        <div class="headind_srch">
                            <div class="recent_heading">
                                <h4>Total Users : <label class="btn btn-primary" id="total_active_user"></label></h4>
                            </div>
                        </div>
...
<div id="hidden-chat-people-template" class="d-none">
    <div class="chat_list" id="user_##USERID##">
        <div class="chat_people">
            <div class="chat_img"> 
                <img src="https://ptetutorials.com/images/user-profile.png" alt="##USERNAME##"> 
            </div>
...
window.addEventListener('load',  () =>{
    ...
    var channel = Echo.join("session-channel-{{ $sessionId }}");
    channel.here((users) => {

        this.usersList = users;
        $('#total_active_user').html(users.length);
        $(".inbox_chat").html('');
        $.each(users, function(index, event) {

            $htmlTemplate = $("#hidden-chat-people-template").html();
            $htmlTemplate = $htmlTemplate.replace(/##USERNAME##/g, event.name);
            $htmlTemplate = $htmlTemplate.replace(/##USERID##/g, event.id);
    ...
    channel.joining((user) => {
        ...
        $('#total_active_user').html(this.usersList.length);
        console.log('channel joining...');
        $htmlTemplate = $("#hidden-chat-people-template").html();
        $htmlTemplate = $htmlTemplate.replace(/##USERNAME##/g, user.name);
        $htmlTemplate = $htmlTemplate.replace(/##USERID##/g, user.id);
